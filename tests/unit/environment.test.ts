...
describe("environment validation", () => {
  ...
  it("requires auth credentials once authentication is enabled", () => {
    expect(() => parseEnvironment({ GRINDLY_STAGE: "auth" })).toThrow(
      "SUPABASE_URL",
    );
  });
  it("requires a deployment and issuer for membership", () => {
    ...
    ).toThrow("MEMBERSHIP_CONTRACT_ADDRESS");
  });
  it("never includes secret values in errors", () => {
    expect(() =>
      parseEnvironment({ ISSUER_PRIVATE_KEY: "sensitive-test-value" }),
    ).toThrow(/^Invalid environment: ISSUER_PRIVATE_KEY$/);
  });
  it("rejects non-HTTP endpoints", () => {
    expect(() => parseEnvironment({ SUPABASE_URL: "file:///secret" })).toThrow(
      "SUPABASE_URL",
    );
  });
});
